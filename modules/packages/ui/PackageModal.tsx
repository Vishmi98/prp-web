...
import { createPackageRequest } from "../packages.service";
import { CreatePackageRequestPayload, PackageModalProps } from "../packages.types";
import { packageFormInitialValues, packageValidationSchema } from "../packages.utils";


export default function PackageModal({ pack, isOpen, onClose }: PackageModalProps) {
    const [isSubmitting, setIsSubmitting] = useState(false);

    const formik = useFormik<CreatePackageRequestPayload>({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                ...
                if (response.success) {
                    toast.success(response.message || "Package request sent successfully!");
                    resetForm();
                    onClose();
                } else {
                ...
                <div className="group relative mt-10 mb-5 overflow-hidden rounded-2xl border border-[#111111]/15 bg-gradient-to-br from-[#fdfbf7] to-[#f5f1ea] p-4 shadow-sm transition-all duration-300 hover:border-[#8a6a12]/40 hover:shadow-xl">
                    ...
                    <div className="flex items-center justify-between">
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-[#8a6a12]/30 bg-[#8a6a12]/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.2em] text-[#8a6a12]">
                            <span className="h-1.5 w-1.5 rounded-full bg-[#8a6a12]"></span>
                            Package Request
                        </span>

                        {/* Optional Category Pill */}
                        {pack.category?.length ? (
                        ...
                        <div>
                            ...
                            {pack.sessionsCount ? (
                                <p className="mt-1 text-xs font-medium uppercase tracking-wider text-[#111111]/60">
                                    Includes {pack.sessionsCount} session{pack.sessionsCount > 1 ? "s" : ""}
                                </p>
                            ) : null}
                        </div>
